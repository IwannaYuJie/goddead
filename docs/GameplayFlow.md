...
这份文档按玩家实际体验拆成四层：基础主线、门厅支线网、终局后章节链、当前最新 v90–v99。源码里的 220 个场景并不是一条直线；`remembrance` 是后半程章节入口与终审入口的总枢纽，多个旧场景会在新章节中被重新征用为回桥。
...
| v95 | 3 钟 × 3 修法 = 9，再加 3 个听证 | 三只钟都修过、三种修法都用过，即开庭；每只钟要拖动表针拨到正确的点才能上弦 | 208 |
| v96 | 3 气 × 3 赎法 = 9，再加 3 个裁定 | 三口气都赎过、三种赎法都用过，即开庭；每口气要按住风箱、在压力区间内松手才送回 | 211 |
| v97 | 3 物 × 3 称法 = 9，再加 3 个裁定 | 三样东西都称过、三种称法都用过，即开庭；每样东西要勾选砝码让秤杆放平才能落秤 | 214 |
...
v94 三项仲裁集齐后开放。三只钟都来自早期主线房间。

```mermaid
flowchart TD
    M[痕迹室：v94 三项仲裁集齐] --> S[停摆钟修理铺\n三只没有表针的钟]
    S -->|值夜室 / 守则板 / 焚献炉| B[修钟台\n选修法，拖动表针，上弦]
    B -->|齿轮咬不上| B
    B -->|咬上了| T{钟原来所在的房间}
    B -->|把钟挂回墙上| S
...
- 刻痕：值夜室五点、守则十二点、焚献炉九点；修法为拨回（刻痕）、走一格（+1）、拨到对面（+6）。
- 表盘可拖动也可用方向键；表针只在内存里，换钟卸针，换修法留针。
- 开庭条件：三只钟 x/3、三种修法 x/3。最短三次。

## 5.6 v96 屏息当铺

v95 三项听证集齐后开放。三口气来自走廊、余响交换台、无主投递所。

```mermaid
flowchart TD
    M[痕迹室：v95 三项听证集齐] --> P[屏息当铺\n三只封气的玻璃瓶]
    P -->|走廊 60% / 交换台 30% / 投递所 80%| B[风箱柜台\n选赎法，按住风箱，在区间内松手]
    B -->|松早 / 松晚| B
    B -->|压力正好| T{那口气原来的房间}
    B -->|把瓶子放回柜台| P
    T --> T1[走廊 · 走廊估价师]
...
- 赎法：原样还给他（长度）、只还一半（一半）、替他屏住（压满）；容差 ±8%，3.6 秒压满。
- 开庭条件：三口气 x/3、三种赎法 x/3。最短三次。

## 5.7 v97 失重局

v96 三项裁定集齐后开放。签收处是回声 / 血管 / 忏悔三条老支线（v29 守卫为 v97 在途称单放行）。

```mermaid
flowchart TD
    M[痕迹室：v96 三项裁定集齐] --> L[失重局\n三样浮着的东西]
    L -->|回声 7 两 / 脉搏 5 两 / 忏悔 9 两| B[天平室\n选称法，勾选砝码，秤平后落秤]
    B -->|秤没平| B
    B -->|秤平了| T{那样东西原来的支线}
    B -->|放回玻璃罩| L
    T --> T1[回声档案室 · 回声秤手]
...
- 称法：称回原重（原重）、加倍奉还（× 2）、补足一斤（16 − 原重）；砝码 1 / 2 / 4 / 8 / 16 两可多选。
- 开庭条件：三样东西 x/3、三种称法 x/3。最短三次。

## 5.8 v98 守夜烛台

v97 三项裁定集齐后开放。签收处是神圣遗物科、神名注销科、代神席。

```mermaid
flowchart TD
    M[痕迹室：v97 三项裁定集齐] --> H[守夜厅\n三架 3×3 铁烛台]
    H -->|遗物科 斜对两角 / 注销科 四个角 / 代神席 正中| B[守夜台\n选守法，点蜡烛翻转，对上后封灯]
    B -->|没对上 / 问守夜人 / 复位| B
    B -->|图案对上| T{烛台原来的房间}
    B -->|放回石台| H
    T --> T1[神圣遗物科 · 遗物守夜人]
...
- 守法：全部熄灭、点成十字、点成一圈；点一支翻转自己和上下左右，9 种组合都有解。
- 开庭条件：三架烛台 x/3、三种守法 x/3。最短三次。

## 5.9 v99 引路司

v98 三项裁定集齐后开放。签收处是门外、访客守则、值夜室。

```mermaid
flowchart TD
    M[痕迹室：v98 三项裁定集齐] --> O[引路司\n三张半截路图]
    O -->|门外 / 守则 / 值夜室| B[铺路台\n选去处，旋转路砖，接通后落定]
    B -->|没接通 / 复位| B
    B -->|接通| T{路图原来的房间}
    B -->|卷起路图| O
...
- 去处：升上去（右上往上）、走出去（右中往右）、沉下去（右下往下）；每块砖点一下顺时针转 90°，接上入口的砖会亮。
- 开庭条件：三张路图 x/3、三个去处 x/3。最短三次。
