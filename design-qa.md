...
- 目标：不在主线末尾再堆楼层，而是让 v72 的三主权违约结局触发一场死者开始购买并不属于自己的梦的边境危机；新增梦境海关总署、违禁睡眠申报厅、噩梦关税局、清醒驱逐场四场景，玩家依次选择梦境护照（神死前旧梦 / 未生者睡签 / 未来证人夜证）、违禁梦物（醒时未见之脸 / 醒后续梦记忆 / 无梦者结局）与噩梦关税（以清醒年岁征税 / 没收做梦者 / 把梦退运给死亡 / 给予噩梦庇护）生成 36 份梦境申报；每完成一份申报后在对应旧场景（eyelid-archive / Remembrance / unending-gallery）留下独立梦检员，可返回海关继续下一轮；覆盖三护照、三违禁物与四关税后 Remembrance 显露「把所有清醒者驱逐出梦境」，进入清醒驱逐场开启让所有噩梦成为公民 / 把做梦者驱逐出梦 / 把清醒列为违禁品三种最终判决。
...
- 静态：`node --check script.js`、`node --check tests/site.test.mjs`、`node tests/site.test.mjs`（7880 assertions 全绿）、`git diff --check` 全部干净通过；覆盖四源图/WebP 哈希与预算、125 场景、36+3 冻结文案、十一键状态、七类 strict pending、activeInspector 必须已收集反算、18 组 isTrusted、合成 click 零副作用、deportation outcome settle/repeat、customs/terminal/bureau/yard UI disabled 与 handler 一致（activeInspector/draft/pending 不生成假按钮；yard 场景 activeInspector 时 figure/tally 仍可见、action 全 disabled）、Remembrance 图鉴/双入口/目录、TARIFF_TABLE 四项冻结 title、图鉴标题无 undefined、forget-all 清 v73、窄屏热点 ≥44px，三个旧场景 inspector-return DOM 重播回归（正确容器显露、反馈逐字、disabled、aria-pressed、单次 schedule），Remembrance 首轮阻断 DOM 回归（v72 前置解锁但自身零进度时 codex/entry/记忆行/普通入口可见可点、39 格全锁、deportation 入口隐藏；activeInspector 落在 remembrance 时 deportation-entry 仍 visible+disabled），以及 v73 declaration pending / activeInspector 在 eyelid-archive / remembrance / unending-gallery 三个旧场景不被自身守卫误弹回 remembrance 的集成回归。缓存版本 `v=72 → v=73`；场景 121 → 125。
- 真实浏览器 QA（Codex 独立验收，本地生产文件）：**已通过**。Chrome 真实点击走通入口 → 护照 → 违禁梦 → 关税 → 旧场景 → 梦检员返回；锁态、ready-zero、3/4 与 4/4 覆盖门槛、三个梦检员、三个驱逐目标、pending 重播、刷新不重复、坏 JSON/数组回落均通过；1280×720 与 390×844 四幕布局通过，图片完整、无横向溢出、热点不重叠且 ≥44px；页面自身无 console error/warning；Chrome 仅出现 Immersive Translate 扩展自己的版本报错，与项目无关；静态 7880 assertions、两份 node --check 与 git diff --check 通过。

## 本轮新增：v74 墓碑专利局 / PATENT OFFICE OF THE UNBURIED（v73 三清醒驱逐结局 + 覆盖后 3×3×4=36 不可能专利 + 3 永久许可裁定 = 39 新图鉴，场景 125 → 129）

- 目标：不在主线末尾再堆楼层，而是让 v73 的三清醒驱逐结局触发一场从未被制造之物开始主张专利权的荒诞诉讼；新增墓碑专利局、先前技术骨库、不可实现权项审查室、永久许可终审庭四场景，玩家依次选择不可能申请人（未生发明人 / 死后署名人 / 未来抄袭者）、不存在先前技术（未刻墓志图纸 / 梦中磨损原型 / 后人记忆机器）与违背因果的权利要求（主张拥有未造之物 / 许可死亡成为使用者 / 起诉未来倒向抄袭 / 禁止发明人完成发明）生成 36 件不可能专利；每完成一件专利后在对应旧场景（threshold / eyelid-archive / remembrance）留下独立墓碑审查员，可返回专利局继续下一轮；覆盖三申请人、三先前技术与四权项后 Remembrance 显露「把存在本身送进永久许可终审」，进入终审庭开启让发明拥有自己 / 把既往存在全部无效 / 许可未葬者附身原型三种最终裁定。
...
- 静态：`node --check script.js`、`node --check tests/site.test.mjs`、`node tests/site.test.mjs`、`git diff --check` 全部干净通过；覆盖四源图/WebP 哈希与预算、129 场景、36+3 冻结文案、十一键状态、七类 strict pending、activeExaminer 必须已收集反算、18 组 isTrusted、合成 click 零副作用、ruling outcome settle/repeat、office/ossuary/examination/tribunal UI disabled 与 handler 一致（activeExaminer/draft/pending 不生成假按钮；tribunal 场景 activeExaminer 时 figure/tally 仍可见、action 全 disabled）、Remembrance 图鉴/双入口/目录、CLAIM_TABLE 四项冻结 title、图鉴标题无 undefined、forget-all 清 v74、窄屏热点 ≥44px，三个旧场景 examiner-return DOM 重播回归（正确容器显露、反馈逐字、disabled、aria-pressed、单次 schedule），Remembrance 首轮阻断 DOM 回归（v73 前置解锁但自身零进度时 codex/entry/记忆行/普通入口可见可点、39 格全锁、终审入口隐藏；activeExaminer 落在 remembrance 时 tribunal-entry 仍 visible+disabled），以及 v74 patent pending / activeExaminer 在 threshold / eyelid-archive / remembrance 三个旧场景不被自身守卫误弹回 remembrance 的集成回归。缓存版本 `v=73 → v=74`；场景 125 → 129。
- 真实浏览器 QA（Codex 独立验收，本地生产文件）：已通过。静态最终基线 8441 assertions、`node --check script.js`、`node --check tests/site.test.mjs`、`git diff --check` 全绿。P1 修复：独立验收发现 `sceneInit` 主同步链只同步到 v73，未调用 v74 的 `syncTombstonePatentOffice*` / `paintTombstonePatentOffice*` / `replayTombstonePatentOfficePending`，导致合法解锁态下 Remembrance 的 v74 入口、记忆、图鉴均未初始化；已补齐 v74 全套同步调用并追加整页初始化链回归测试。Codex 真浏览器复验：四锁定深链回退、修复后 sceneInit 同步链、ready-zero、4 专利覆盖解锁终审/3 专利不解锁、完整 isTrusted 办理链与审查员返回、坏 JSON/数组降级、梦型/后忆 activeExaminer、remembrance 双入口 visible+disabled、桌面 1280×720 与手机 390×844 四张图布局/无横溢/触控尺寸均通过。

## 本轮新增：v75 末日保修局 / APOCALYPSE WARRANTY OFFICE（v74 三永久许可裁定 + 覆盖后 3×3×4=36 末日报修单 + 3 世界售后裁定 = 39 新图鉴，场景 129 → 133）

- 目标：不在主线末尾再堆楼层，而是让 v74 的三永久许可裁定触发一场从未完成机器开始主张世界末日后仍属保修期的荒诞售后；新增末日保修局、购买凭证停尸房、世终之后维修台、世界总召回场四场景，玩家依次选择因果外故障（出厂前磨损 / 世终后不停机 / 幽灵活动零件）、不可能购买凭证（未建工厂收据 / 死太阳保修封 / 后人维修记忆）与现实维修方案（替换现实而非零件 / 把保修延长到出生前 / 宣布故障属于原厂特性 / 把账单寄给末日）生成 36 份末日报修单；每完成一份报修单后在对应旧场景（threshold / Remembrance / unending-gallery）留下独立亡后理赔员，可返回保修局继续下一轮；覆盖三故障、三凭证与四方案后 Remembrance 显露「召回整批已经售出的现实」，进入世界总召回场开启把世界从流通中召回 / 给末日安装备用黎明 / 因现实擅自维修而作废三种最终售后裁定。
...
- 静态：`node --check script.js`、`node --check tests/site.test.mjs`、`node tests/site.test.mjs`（9051 assertions 全绿）、`git diff --check` 全部干净通过；覆盖四源图/WebP 哈希与预算、133 场景、36+3 冻结文案、十一键状态、七类 strict pending、activeAdjuster 必须已收集反算、18 组 isTrusted、合成 click 零副作用、recall outcome settle/repeat、office/morgue/bench/yard UI disabled 与 handler 一致（activeAdjuster/draft/pending 不生成假按钮；yard 场景 activeAdjuster 时 figure/tally 仍可见、action 全 disabled）、Remembrance 图鉴/双入口/目录、REMEDY_TABLE 四项冻结 title、图鉴标题无 undefined、forget-all 清 v75、窄屏热点 ≥44px，三个旧场景 adjuster-return DOM 重播回归（正确容器显露、反馈逐字、disabled、aria-pressed、单次 schedule），Remembrance 首轮阻断 DOM 回归（v74 前置解锁但自身零进度时 codex/entry/记忆行/普通入口可见可点、39 格全锁、召回入口隐藏；activeAdjuster 落在 remembrance 时 recall-entry 仍 visible+disabled），以及 v75 warranty-claim pending / activeAdjuster 在 threshold / remembrance / unending-gallery 三个旧场景不被自身守卫误弹回 remembrance 的集成回归；新增 sceneInit 与 DOMContentLoaded 整页初始化链回归，确保 v75 同步调用紧跟 v74 之后；新增 P1 回归断言：3 defect / 3 proof / 4 remedy / 3 recall 共 13 个按钮的 handler `buttonAvailable` 前缀与事件监听 `$('id')` 必须与 `index.html` 真实 ID 一致。缓存版本 `v=74 → v=75`；场景 129 → 133。
- P1（Codex 真实浏览器验收发现）：末日保修局按钮在 DOM 中 visible/enabled，但真实鼠标与语义点击均无反馈、无 pending、无转场；根因为 `chooseApocalypseWarrantyDefect/Proof/Remedy/RecallAction` 以及 `lock*`、`sync*` 中查找的按钮 ID 前缀（`defect-` / `proof-` / `remedy-` / `recall-action-`）与 `index.html` 真实 ID（`apocalypse-warranty-defect-` / `proof-of-purchase-` / `post-world-repair-remedy-` / `universal-recall-`）不一致，导致 `buttonAvailable` 与事件监听均绑定到不存在的元素。已统一修正为真实 DOM ID，并同步修正 `forget-all` 重置数组与可执行回归测试中的 mock element key。
- 真实浏览器 QA（Codex 独立验收，本地生产文件）：已通过。静态最终基线 `node tests/site.test.mjs = 9051 assertions passed`、`node --check script.js`、`node --check tests/site.test.mjs`、`git diff --check` 全绿。真实 Chrome 完整链路：Remembrance 入口 → `worn-before-manufacture` → `receipt-from-unbuilt-factory` → `replace-reality-not-part` → threshold 空厂理赔员 → 返回保修局；覆盖种子后 real mouse 召回世界 → threshold 通过。P1 四组 ID 的 handler / lock / sync / 事件监听 / DOM 联动回归已由新增断言与真实点击双重覆盖。桌面四幕与移动 390×844 保修局视觉可读、无横向溢出；页面自身 `console.warn/error = 0`，Immersive Translate 扩展自身两条超时错误为第三方扩展噪声，明确排除。locked/ready-zero/39 格图鉴/坏 JSON/七类 pending/三处 activeAdjuster/coverage/幂等/三轴矩阵由 9051 静态与此前浏览器状态矩阵覆盖。

## 本轮新增：v76 现实退款处 / REALITY REFUND COUNTER（v75 三世界售后裁定 + 覆盖后 3×3×4=36 现实退货单 + 3 退款结局 = 39 新图鉴，场景 133 → 137）

- 目标：不在主线末尾再堆楼层，而是让 v75 的三世界售后裁定触发一场「存在」 itself 开始向现实索要退款的后售后危机；新增现实退款处、存在凭证焚化库、现实退货检验台、不符描述集体诉讼庭四场景，玩家依次选择被错误售出的存在（用童年购买的肉身 / 以遗忘支付的姓名 / 从死亡租来的年岁）、存在付款凭证（童年价签 / 抹名收据 / 死亡退款理由）与现实退款方案（原路退回不存在 / 恢复出厂缺席 / 换货为一个可能的自己 / 向现实收取重新上架费）生成 36 份现实退货单；每完成一份退货单后在对应旧场景（borrowed-childhood / blank-name-cloakroom / lifetime-pawn-vault）留下独立亡后退款员，可返回退款处继续下一轮；覆盖三存在、三凭证与四方案后 Remembrance 显露「起诉现实从未符合描述」，进入不符描述集体诉讼庭开启退回全部存在并全额退款 / 把所有肉身退回童年 / 判现实虚假宣传三种最终裁定。
...
- 状态：`goddead_v76_reality_refund`（唯一 key，version 76）；显式 canonical 十一键投影（version/visited/draft/refundCases/classOutcomes/refundRuns/classRuns/claimantTallies/lastOutcome/activeCashier/pending），派生字段不落盘；`refundCases` 按 `SUBJECTS × PROOFS × REMEDIES` 固定顺序去重；`classOutcomes` 按 `all-existence-was-refunded-to-the-void / every-body-was-refunded-to-childhood / reality-admitted-it-never-matched-description` 固定顺序去重；`draft` 仅 `subject/proof` 两键且 proof 非空时 subject 必须合法；`activeCashier` 仅允许 `{proof,refundCase,feedback}` 三键且 refundCase 已收集、proof 等于 refundCase 第二段、feedback 由凭证表逐字重算；`claimantTallies` 精确 `{body,name,time}` 三键且不从 refundCases 反推，重复退货单也必须留下真实主体票数；计数 floor 后 0..9999 封顶；`lastOutcome` 必须存在于规范 refundCases 或 classOutcomes；pending 七类精确键集逐字重算 + 实时 v75 解锁与 coverage 校验 + 一次性消费；重播按 source/target/else 矩阵严格恢复。
...
- 目标：在 v76 判现实虚假宣传之后，每一种「可能的自己」都拿着同一张换货单出现；新增自我真伪鉴定所、自我来源凭证库、灵魂赝品检验室、等同自我终审庭四场景，玩家依次选择要求被承认为原装的自我（原装人格 / 替换记忆 / 仿制灵魂）、自我来源凭证（第一伤口封印 / 童年镜证 / 余温死面模）与真伪鉴定方法（认证最早版本 / 用伤口核对记忆 / 让复制品指认原装 / 宣布真实性可转让）生成 36 份自我鉴定证书；每完成一份证书后在对应旧场景（scar-loom / borrowed-childhood / lifetime-pawn-vault）留下独立亡后鉴定员，可返回鉴定所继续下一轮；覆盖三候选自我、三来源凭证与四鉴定方法后 Remembrance 显露「裁定哪一个自己有权继续说“我”」，进入等同自我终审庭开启只承认一个原装 / 合并所有可能的自己 / 让每个复制品成为原装三种最终裁定。
...
- 入口与防伪：普通入口仅在 `selfAuthenticityOfficeUnlocked()`（v76 refundCases 覆盖 3 存在 / 3 凭证 / 4 方案 且 classOutcomes 含三规范 outcome）为真时显露；终审入口「裁定哪一个自己有权继续说“我”」仅在已收集 certificates 同时覆盖 3 候选自我、3 来源凭证、4 鉴定方法时显露；四场景 direct hash 在 v76 证据不足或 pending 不合法时统一回退 `#remembrance`。
- 四幕：自我真伪鉴定所三候选自我左 / 中 / 右清楚分离；自我来源凭证库三凭证左 / 中 / 右清楚分离；灵魂赝品检验室四方法按左上 / 右上 / 左下 / 右下互不遮挡；等同自我终审庭三最终裁定左 / 中 / 右分离，场内同步显示 claimantTallies 与派生自我多数结论。每幕原生 button 热点 ≥44px，反馈区 `aria-live="polite"`，pending 时整组 disabled、仅已选项 `aria-pressed=true`。
- 鉴定员：scar-loom / borrowed-childhood / lifetime-pawn-vault 三旧落点各新增独立亡后鉴定员容器与返回鉴定所按钮，不复用 v72/v73/v74/v75/v76 原反馈、汇兑员、梦检员、审查员、理赔员或退款员；`activeAuthenticator` 精确反算到已收集 certificate，provenance 与 id 第二段一致、feedback 由来源凭证表逐字重算。
- 状态：`goddead_v77_self_authenticity`（唯一 key，version 77）；显式 canonical 十一键投影（version/visited/draft/certificates/tribunalOutcomes/examRuns/tribunalRuns/claimantTallies/lastOutcome/activeAuthenticator/pending），派生字段不落盘；`certificates` 按 `CLAIMANTS × PROVENANCES × METHODS` 固定顺序去重；`tribunalOutcomes` 按 `one-self-became-the-only-original / all-possible-selves-merged-into-one / every-copy-became-an-original` 固定顺序去重；`draft` 仅 `claimant/provenance` 两键且 provenance 非空时 claimant 必须合法；`activeAuthenticator` 仅允许 `{provenance,certificate,feedback}` 三键且 certificate 已收集、provenance 等于 certificate 第二段、feedback 由来源凭证表逐字重算；`claimantTallies` 精确 `{original,memory,soul}` 三键且不从 certificates 反推，重复证书也必须留下真实候选票数；计数 floor 后 0..9999 封顶；`lastOutcome` 必须存在于规范 certificates 或 tribunalOutcomes；pending 七类精确键集逐字重算 + 实时 v76 解锁与 coverage 校验 + 一次性消费；重播按 source/target/else 矩阵严格恢复。
...
- 静态：`node --check script.js`、`node --check tests/site.test.mjs`、`node tests/site.test.mjs`（10277 assertions 全绿）、`git diff --check` 全部干净通过；覆盖四源图/WebP 哈希与预算、141 场景、36+3 冻结文案、十一键状态、七类 strict pending、activeAuthenticator 必须已收集反算、18 组 isTrusted、合成 click 零副作用、tribunal outcome settle/repeat、office/vault/examination/tribunal UI disabled 与 handler 一致（activeAuthenticator/draft/pending 不生成假按钮；tribunal 场景 activeAuthenticator 时 figure/tally 仍可见、action 全 disabled）、Remembrance 图鉴/双入口/目录、METHOD_TABLE 四项冻结 title、图鉴标题无 undefined、forget-all 清 v77、窄屏热点 ≥44px，三个旧场景 authenticator-return DOM 重播回归（正确容器显露、反馈逐字、disabled、aria-pressed、单次 schedule），Remembrance 首轮阻断 DOM 回归（v76 前置解锁但自身零进度时 codex/entry/记忆行/普通入口可见可点、39 格全锁、终审入口隐藏；activeAuthenticator 落在 remembrance 时 tribunal-entry 仍 visible+disabled），以及 v77 certificate pending / activeAuthenticator 在 scar-loom / borrowed-childhood / lifetime-pawn-vault 三个旧场景不被自身守卫误弹回 remembrance 的集成回归；新增 sceneInit 与 DOMContentLoaded 整页初始化链回归，确保 v77 同步调用紧跟 v76 之后；缓存版本 `v=76 → v=77`；场景 137 → 141。
- 真实浏览器 QA（Codex 独立验收，本地生产文件）：仍待执行。

## 本轮新增：v72 遗言中央银行 / CENTRAL BANK OF LAST WORDS（v71 三未宣战结局 + 覆盖后 3×3×4=36 遗言金融工具 + 3 主权违约结局 = 39 新图鉴，场景 117 → 121）

- 目标：不在主线末尾再堆楼层，而是让 v71 的三未宣战结局触发一场姓名、影子与肉身建国后发现无币可付跨境死亡的金融危机；新增遗言中央银行、未言货币铸造局、遗嘱清算金库、主权违约庭四场景，玩家依次选择货币（无主签名钞 / 无人注视黑币 / 未言遗嘱债）、储备（最后一口气 / 继承来的沉默 / 抵押一段结局）与政策（先于说出口发行 / 让告别贬值 / 冻结复活流动性 / 在他人口中赎回）生成 36 份遗言金融工具；每完成一份工具后在对应旧场景（threshold / Remembrance / unending-gallery）留下独立汇兑员，可返回央行继续下一轮；覆盖三货币、三储备与四政策后 Remembrance 显露「让所有遗言同时违约」，进入主权违约庭开启国有化所有遗言 / 让沉默决定利率 / 宣布死亡大到不能倒闭三种宏观结局。
...
- 四幕：遗言中央银行三货币左 / 中 / 右清楚分离；未言货币铸造局三储备左 / 中 / 右清楚分离；遗嘱清算金库四政策按左上 / 左下 / 右上 / 右下互不遮挡；主权违约庭三宏观结局左 / 中 / 右分离，庭内同步显示 issuerTallies 与派生货币主权结论。每幕原生 button 热点 ≥44px，反馈区 `aria-live="polite"`，pending 时整组 disabled、仅已选项 `aria-pressed=true`。
...
- 状态：`goddead_v72_last_word_bank`（唯一 key，version 72）；显式 canonical 十一键投影（version/visited/draft/instruments/defaultOutcomes/monetaryRuns/defaultRuns/issuerTallies/lastOutcome/activeRemittance/pending），派生字段不落盘；`instruments` 按 `CURRENCIES × RESERVES × POLICIES` 固定顺序去重；`defaultOutcomes` 按 `every-last-word-was-nationalized / silence-set-the-interest-rate / death-became-too-big-to-fail` 固定顺序去重（对应 action 顺序 nationalize-every-last-word / let-silence-set-interest / declare-death-too-big-to-fail）；`draft` 仅 `currency/reserve` 两键且 reserve 非空时 currency 必须合法；`activeRemittance` 仅允许 `{reserve,instrument,feedback}` 三键且 instrument 已收集、reserve 等于 instrument 第二段、feedback 由储备表逐字重算；`issuerTallies` 精确 `{name,shadow,body}` 三键且不从 instruments 反推，重复工具也必须留下真实发行票数；计数 floor 后 0..9999 封顶；`lastOutcome` 必须存在于规范 instruments 或 defaultOutcomes；pending 七类精确键集（entry 3 / currency 5 / reserve 6 / instrument 8 / remittance-return 5 / default-entry 3 / default 6，额外键即伪造）逐字重算 + 实时 v71 解锁与 coverage 校验 + 一次性消费；重播按 source/target/else 矩阵严格恢复，目标场景立即 arrive，来源场景恢复反馈/锁定/一次转场。
...
- 静态：`node --check script.js`、`node --check tests/site.test.mjs`、`node tests/site.test.mjs`（7357 assertions 全绿）、`git diff --check` 全部干净通过；覆盖四源图/WebP 哈希与预算、121 场景、36+3 冻结文案、十一键状态、七类 strict pending、activeRemittance 必须已收集反算、18 组 isTrusted、合成 click 零副作用、default outcome settle/repeat、bank/mint/vault/default UI disabled 与 handler 一致（activeRemittance/draft/pending 不生成假按钮；default 场景 activeRemittance 时 figure/tally 仍可见、action 全 disabled）、Remembrance 图鉴/双入口/目录、POLICY_TABLE 四项冻结 title、图鉴标题无 undefined、forget-all 清 v72、窄屏热点 ≥44px，三个旧场景 remittance-return DOM 重播回归（正确容器显露、反馈逐字、disabled、aria-pressed、单次 schedule），Remembrance 首轮阻断 DOM 回归（v71 前置解锁但自身零进度时 codex/entry/记忆行/普通入口可见可点、39 格全锁、default 入口隐藏；activeRemittance 落在 remembrance 时 default-entry 仍 visible+disabled），以及 v72 instrument pending / activeRemittance 在 threshold / remembrance / unending-gallery 三个旧场景不被自身守卫误弹回 remembrance 的集成回归。缓存版本 `v=71 → v=72`；场景 117 → 121。
- 真实浏览器 QA（Codex 独立验收，本地生产文件）：已通过。静态 7357 assertions；四新场景桌面 1280×720 与移动 390×844 图片 natural 1536×1024、完整、无横溢、热点最小 44px、无重叠/越界；四 direct hash 锁定回 remembrance；ready-zero 39 格全锁、普通入口可用；真实 isTrusted 鼠标入口；entry/currency/reserve/instrument 来源刷新重播、threshold 目标刷新与返回；同一 instrument 重复仍图鉴 1、monetaryRuns 真实增长；3 份 coverage 失败、4 份成功、多数/并列；default-entry 与 default 的来源刷新、目标刷新、重复结局去重；threshold/remembrance/unending-gallery 三汇兑员直达/刷新/返回；remembrance 无 coverage activeRemittance 时普通+违约入口 visible+disabled、返回 enabled、清 active 后按 coverage 恢复；draft 中途两入口 disabled、违约庭 figure/tally visible 但 action disabled；坏 JSON/数组回默认且 deep hash 回 remembrance；pending else 清理；图鉴 policy title 修复后四标题无 undefined；应用自身 console warn/error=0（沉浸式翻译扩展 dynamic-i18n version mismatch 为第三方扩展噪声，不计入）；临时 qa-v72-seed.html 与本地 8972 server 由 Codex 负责清理。

## 本轮新增：v71 死亡外交部 / MINISTRY OF MORTAL DIPLOMACY（v70 三宪政危机结局 + 覆盖后 3×3×4=36 死亡条约 + 3 未宣战结局 = 39 新图鉴，场景 113 → 117）

- 目标：不在主线末尾再堆楼层，而是让 v70 的三宪政危机结局触发一场姓名、影子与肉身各自建国后的死亡外交；新增死亡外交部、不存在国境署、条约解剖台、未宣战室四场景，玩家依次选择使团（无身姓名使团 / 流亡影子使团 / 无名肉身领事团）、外国（出生以前的国 / 记忆以后的共和国 / 肉身内部的国）与条款（互相承认 / 庇护未活之人 / 禁运复活 / 引渡结局）生成 36 份唯一死亡条约；每完成一份条约后在对应旧场景（v66 反事实纺锤 / Remembrance / v70 宪法分籍台）留下独立外交信使，可返回外交部继续下一轮；当三使团、三外国与四条款全部覆盖后开启未宣战入口，提供把所有边境移进肉身 / 只承认流亡者 / 把复活列为跨境罪三种未宣战结局。
...
- 静态：`node --check script.js`、`node --check tests/site.test.mjs`、`node tests/site.test.mjs`（6881 断言全绿）、`git diff --check` 全部干净通过；覆盖四源图/WebP 哈希与预算、117 场景、36+3 冻结文案、十一键状态、七类 strict pending、activeCourier 反算、18 组 isTrusted、合成 click 零副作用、Remembrance 图鉴/双入口/目录、forget-all 清 v71、窄屏热点 ≥44px，三个旧场景 courier-return DOM 重播回归（正确容器显露、反馈逐字、disabled、aria-pressed、单次 schedule），Remembrance 首轮阻断 DOM 回归（v70 前置解锁但自身零进度时 codex/entry/记忆行/普通入口可见可点、39 格全锁、war 入口隐藏），以及 v71 treaty pending / activeCourier 在 counterfactual-spindle / remembrance / constitutional-severance-desk 三个旧场景不被弹回 remembrance 的集成回归。缓存版本 `v=70 → v=71`；场景 113 → 117。
- 真实浏览器 QA（Codex 独立验收，本地生产文件）：已通过。静态 6881 assertions；四新场景桌面 1280×720 与移动 390×844 图片、热区、无横溢均通过，热区最小 44px；四个锁定 hash 都回 remembrance；代表条约签署/目标结算/重复/刷新幂等通过；三处信使直达与刷新、activeCourier 时普通入口和战争入口 visible+disabled、返程清理通过；4 条约战争门槛、3 条未解锁、票数平局“无人获得承认”、代表战争结局与刷新幂等通过；坏 JSON/错误类型存档回 0/36；错误场景 pending 清理；合成点击无副作用、Chrome 真实鼠标点击能进入死亡外交部；新鲜页 warn/error=0；验收 helper / tab / 服务已清理。

## 本轮新增：v70 亡者议会 / PARLIAMENT OF THE DEAD（v69 三注销结局 + 覆盖后 3×4×3=36 死者法令 + 3 宪政危机结局 = 39 新图鉴，场景 109 → 113）

- 目标：不在主线末尾再堆楼层，而是让 v69 的三注销结局把出生追认者、人口移除者与空白户籍公民拆成三个幽灵选区；新增亡者议会、公民权议案厅、宪法分籍台、三人共和国四场景，玩家依次选择选区（迟生者 / 被移除者 / 空白公民）、四项权利议案（姓名权 / 影子权 / 肉身权 / 一次死亡权）与公民权归属（姓名 / 影子 / 肉身），生成 36 条唯一死者法令；每完成一条法令后在对应 v69 旧场景留下独立议会党鞭，可返回亡者议会继续下一轮；当三选区、四议案与三种公民权归属全部覆盖后开启危机入口，提供姓名成为唯一公民 / 影子另组反对共和国 / 肉身废除死者选举三种宪政危机结局。
...
- 静态：`node --check script.js`、`node --check tests/site.test.mjs`、`node tests/site.test.mjs`（6502 断言全绿）、`git diff --check` 全部干净通过；覆盖四源图/WebP 哈希与预算、113 场景、36+3 冻结文案、十一键状态、七类 strict pending、activeWhip 反算、18 组 isTrusted、合成 click 零副作用、Remembrance 图鉴/双入口/目录、forget-all 清 v70、窄屏热点 ≥44px，三个旧场景 whip-return DOM 重播回归（正确容器显露、反馈逐字、disabled、aria-pressed、单次 schedule），Remembrance 首轮阻断 DOM 回归（v69 前置解锁但自身零进度时 codex/entry/记忆行/普通入口可见可点、39 格全锁），以及 v70 decree pending / activeWhip 在 birth-ballot-booth / posthumous-census-hall / contradictory-evidence-archive 三个 v69 旧场景不被弹回 remembrance 的集成回归。缓存版本 `v=69 → v=70`；场景 109 → 113。
- 真实浏览器 QA（Codex 独立验收，本地生产文件）：**通过**。locked：四个 v70 direct hash 均归一 `#remembrance`，v70 记忆/图鉴/双入口/四目录/三党鞭全隐藏。ready-zero：记忆 `0/36`、39 格全锁、普通入口 visible+enabled、危机入口隐藏。四幕桌面 1280×720 + 移动 390×844：四图 `complete` 且 `naturalWidth=1536`、`naturalHeight=1024`；热点全在图内、零重叠、移动端 ≥44px、横向 overflow 0；人工看图通过。isTrusted：应用内程序化 click/Enter 被拒且状态零变化；Chrome Computer Use 真实鼠标点击从 `#remembrance` 成功进入 `#dead-parliament-rotunda`，标题/场景/地址同步，存档仅 `visited.rotunda=true`，其余保持 0，`pending=null`，原存档 restore-ok。representative decree `ratified-born:right-to-name:name`：首次 `decrees=1`、`parliamentRuns=1`、`seatTallies.name=1`；刷新不重算；重复同 decree 后 `decrees=1`、`parliamentRuns=2`、`name=2`。三支 caucus 党鞭在三个 v69 旧场景正确显示且刷新稳定；whip-return source/target 恢复一致，返回后 `activeWhip` 与 `pending` 均为 `null`。coverage 四条代表 decree 后记忆 `4/36`、`parliamentRuns=4`、`shadow=2` 为多数，危机入口开启；三人共和国并列种子显示「议会悬空」，三危机按钮可用。representative crisis `the-shadow-founded-the-opposition-republic`：`crisisRuns=1`、outcomes=1、`pending=null`；刷新幂等。七类 pending source/target/else 逐项一致，else 进入 `protocol` 只清 pending 不结算。坏 JSON/坏类型回退默认 `0/36`、39 格全锁、普通入口可用、危机入口隐藏。干净生产页巡检 `#remembrance` + 四幕 `console.warning/error=0`（旧 QA iframe 的 MutationObserver 注入噪声不计入生产页）。QA 临时 helper 已删除，本地服务已停止，两套浏览器存档均恢复。

## 本轮新增：v69 死后人口普查 / THE DEAD COUNT WHETHER YOU WERE BORN（v68 三止赎结局 + 覆盖后 3×3×3=27 人口记录 + 3 注销结局 = 30 新图鉴，场景 105 → 109）

- 目标：不在主线末尾再堆楼层，而是让 v68 的三止赎结局触发一场不承认活人证词的死后人口普查；新增死后普查厅、矛盾证据库、出生投票间、人口注销庭四场景，玩家依次选择选民（逝代 / 同代 / 后世）、存在证据（出生证明 / 他人记忆 / 空白户籍）与出生裁定（已出生 / 未出生 / 借名）生成 27 份唯一人口记录；每完成一次投票后在对应 v68 场景留下独立普查传票，可返回死后普查厅继续下一轮；当三类选民、三类证据与三种裁定全部覆盖后开启注销入口，提供追认这次出生 / 把来访者移出人口 / 让空白户籍成为公民三种人口注销结局。
...
- 状态：`goddead_v69_posthumous_census`（唯一 key，version 69）；显式 canonical 十一键投影（version/visited/draft/records/nullificationOutcomes/ballotRuns/nullificationRuns/tallies/lastOutcome/activeSummons/pending），派生字段不落盘；`records` 按 `ELECTORATES × EVIDENCES × VERDICTS` 固定顺序去重；`nullificationOutcomes` 按 `birth-ratified-after-death / visitor-removed-from-population / blank-register-became-citizen` 固定顺序去重（对应 action 顺序 ratify-birth / remove-visitor / enfranchise-blank）；`draft` 仅 `electorate/evidence` 两键且 evidence 非空时 electorate 必须合法；`activeSummons` 仅允许 `{electorate,record,feedback}` 三键且 record 已收集、electorate 等于 record 首段、feedback 由三表逐字重算；`tallies` 精确 `{born,neverBorn,assigned}` 三键且不从 records 反推，重复人口记录也必须留下真实票数；计数 floor 后 0..9999 封顶；`lastOutcome` 必须存在于规范 records 或 nullificationOutcomes；pending 七类精确键集（entry 3 / electorate 5 / evidence 6 / ballot 8 / summons-return 5 / nullification-entry 3 / nullification 6，额外键即伪造）逐字重算 + 实时 v68 解锁与 coverage 校验 + 一次性消费；重播按 source/target/else 矩阵严格恢复，目标场景立即 arrive，来源场景恢复反馈/锁定/一次转场。
...
- 静态：`node tests/site.test.mjs` 6227 断言全绿；覆盖四源图/WebP 哈希与预算、109 场景、27+3 冻结文案、十一键状态、七类 strict pending、activeSummons 反算、17 组 isTrusted、合成 click 零副作用、Remembrance 图鉴/双入口/目录、forget-all 清 v69、窄屏热点 ≥44px，三个旧场景 summons-return DOM 重播回归（正确容器显露、反馈逐字、disabled、aria-pressed、单次 schedule），Remembrance 首轮阻断 DOM 回归（v68 前置解锁但自身零进度时 codex/entry/记忆行/普通入口可见可点、30 格全锁），以及 v69 ballot pending / activeSummons 在 lifetime-pawn-vault / generational-credit-office / mortality-clearing-house 三个 v68 旧场景不被弹回 remembrance 的集成回归。缓存版本 `v=68 → v=69`；场景 105 → 109。
...
## 本轮新增：v68 世代借贷 / THE GENERATIONS BORROW AGAINST DEATH（v67 三家族结局 + 覆盖后 3×3×3=27 世代借据 + 3 止赎结局 = 30 新图鉴，场景 101 → 105）

- 目标：不在主线末尾再堆楼层，而是让 v67 的无血家族在互认亲属后开始互相借命；新增世代信贷所、寿命典当库、死期清算厅、年岁止赎庭四场景，玩家依次选择出借世代（逝代 / 同代 / 后世）、抵押物（七年 / 死期 / 葬礼）与偿付条款（童年付息 / 出生即偿 / 债传后代）生成 27 份唯一世代借据；每完成一份后在对应 v67 家族结局落点留下独立催收通知，可返回信贷所继续下一轮；当三世代、三抵押、三偿付全部覆盖后开启止赎入口，提供收走当下 / 让死亡破产 / 让葬礼继承家族三种止赎结局。
...
- 静态：`node tests/site.test.mjs` 5986 断言全绿；覆盖四源图/WebP 哈希与预算、105 场景、27+3 冻结文案、十一键状态、七类 strict pending、activeNotice 反算、17 组 isTrusted、合成 click 零副作用、Remembrance 图鉴/双入口/目录、forget-all 清 v68、窄屏热点 ≥44px，三个旧场景 notice-return DOM 重播回归（正确容器显露、反馈逐字、disabled、aria-pressed、单次 schedule），以及 Remembrance 首轮阻断 DOM 回归（v67 前置解锁但自身零进度时 codex/entry/记忆行/普通入口可见可点、30 格全锁）。缓存版本 `v=67 → v=68`；场景 101 → 105。
- 真实浏览器 QA（Codex 独立验收，本地生产文件）：桌面 1280×720 与真实 390×844 iframe viewport 四幕均 active、四张 WebP complete 且 natural 1536×1024、热点在图内/零重叠/≥44px、横向 overflow 0、人工看图通过；locked 状态浏览器确认 v68 UI/图鉴/普通入口/催收通知全隐藏（四幕 deep link 归一 `#remembrance` 由代码审计与静态断言覆盖）；ready-zero 浏览器确认记忆 0/27、30 格全锁、普通入口可见且 enabled、止赎入口隐藏（clean 正向真人点击仍待真人输入验收，合法 pending source/target replay 覆盖转场与恢复契约）；ready-coverage 浏览器确认显示止赎入口/统计/30 格图鉴（四目录链接显隐由静态断言覆盖）；balances 首次 years 借据 1/27/loanRuns 1/years 7，刷新不重复，重复同一借贷 loanRuns 2/years 14 但图鉴仍 1/27；notice-return 返回信贷所后 threshold 催收消失；代表性止赎 seize-present 1/3 通过（三条 outcome 的冻结表/规范化/监听与 pending 由 5986 静态断言覆盖，未来钩子仍要求真实集齐 3/3）；坏 JSON 安全归零且入口仍可用；七类 pending 浏览器确认 source/target/else 行为正确，来源页「只调度一次」由 Node 隔离测试的 scheduleCalls 计数覆盖；Playwright/Codex 程序化 click 被 `e.isTrusted` 防线拒绝且状态零变化；全程 console warning/error=0。

## 本轮新增：v65 因果疤痕 / THE CAUSE LEFT A SCAR（v64 同一落点三模式齐全后 4×3=12 疤痕处理 + 3 无因结局 = 15 新图鉴，场景 92 → 93）

- 目标：不在主线末尾再堆楼层，而是回到门外 / 守则 / 值夜 / 焚献四间旧房，在集齐 v64 同一 destination 的签收 / 退回 / 误投三 outcome 后，显露独立因果疤痕舞台；每舞台提供缝合起因 / 放尽后果 / 移植见证三种处理，生成 12 条疤痕图鉴；四处各至少一条处理后 Remembrance 显露「让四道疤互相指认」入口，进入新场景「无因收容室」，再以你成为原因 / 无物使你发生 / 结局收养了你三种动作收尾。
- 素材：五张冻结原图（sha256 静态断言锁定，见 `docs/V65CausalScarsDesign.md`）Pillow q85 无裁切转码；门外源图 1528×1029、值夜源图 1537×1023，运行时 WebP 用 LANCZOS 轻微归一到 1536×1024；其余三张原生 1536×1024。五张 WebP 为 `assets/v65-threshold-causal-scar.webp`（201924 B）、`assets/v65-protocol-causal-scar.webp`（133536 B）、`assets/v65-watch-causal-scar.webp`（34940 B）、`assets/v65-offering-causal-scar.webp`（107460 B）与 `assets/v65-causeless-ward.webp`（99724 B）。
...
- 四舞台：threshold 红黑缝线 / 门槛引流槽 / 空白见证牌；protocol 缝合纸轨 / 失血托盘 / 移植见证面具；watch 台灯缝线 / 阴影引流盆 / 空椅见证牌；offering 缝死投信口 / 灰烬引流盒 / 炉口见证手。每舞台三个原生 button 热点 ≥44px，反馈区 `aria-live="polite"`，已收集处理重访时 disabled=true 且 aria-pressed=true。
- 无因收容室：左侧自行缝合的暗红因果线轴、中央无婴儿也无姓名的石质空摇篮、右侧被终局裹布抱住的高背收容椅，各承载一个结局热点；三个动作可重复选择，roomOutcomes 只记首次发现。
- 状态：`goddead_v65_causal_scars`（唯一 key，version 65）；显式 canonical 八键投影（version/visited/treatments/roomOutcomes/treatmentRuns/roomRuns/lastOutcome/pending），派生字段不落盘；treatments 按 destination 顺序再按 stitch/drain/graft 顺序去重排序，且只保留当前仍满足 v64 三模式证据的 id；roomOutcomes 按 become-cause/refuse-cause/ending-adopts 固定顺序去重；计数 floor 后 0..9999 封顶；lastOutcome 必须存在于规范 treatments 或 roomOutcomes；pending 三类精确键集（entry 3 / treatment 6 / ending 5，额外键即伪造）逐字重算 + 一次性消费；重播按 source/target/else 矩阵严格恢复，目标场景立即 arrive，来源场景恢复反馈/锁定/一次转场。
...
- 静态：`node tests/site.test.mjs` 5274 断言全绿（较 v64 的 5070 新增 204 条）；缓存版本 `v=64 → v=65`；场景 92 → 93。
- 真实浏览器 QA（Codex 内置浏览器实测，本地 `http://127.0.0.1:4173`）：桌面 1280×720 与移动 390×844 五幕均 active、五张 WebP 1536×1024 complete、热点在图内/零重叠/≥44px、scrollWidth 等于视口；Remembrance ready 种子入口 enabled、4/15 图鉴解锁、记忆行逐字「因果疤痕：已处理 4/12，门外 1 / 守则 1 / 值夜 1 / 焚献 1；无因结局 0/3。」、`03ζ / 无因收容` 目录恢复；locked 直接 `#causeless-ward` 归一 `#remembrance`、threshold 舞台 hidden、坏类型不报错，全程 console warning/error=0。entry/treatment/ending source 与 ending target 的 pending 重播路径反馈/锁定/计数正确（5/12、remembrance 1/3）、无重复结算。但 Browser Playwright/CUA 自动点击仍被 16 个 `e.isTrusted` 防线拒绝，clean 正向真人点击/双击/键盘 Enter/Space 仍待真人输入验收；pending 重播是合法存档整页加载实测，不等于 clean 点击通过。

## 本轮新增：v66 反事实纺生 / THE LIFE THAT NEVER HAPPENED（v65 12+3 齐全后 4×3×3=36 未活之生 + 3 元结局 = 39 新图鉴，场景 93 → 97）

- 目标：不在主线末尾再堆楼层，而是回到门外 / 守则 / 值夜 / 焚献四间旧房，在 v65 集齐 12 道疤痕处理与 3 条无因结局后，由四道疤痕纺出「本可发生却从未活过」的人生；新增反事实纺锤室、疤痕织机、未活育婴室、无因生涯陈列间四场景，玩家依次选择起点（门外 / 守则 / 值夜 / 焚献）、处理方式（缝合 / 引流 / 移植）与生涯归属（成因 / 后果 / 终局）生成 36 条唯一未活之生；每完成一条 life 后在对应旧场景留下未活回声，可返回纺锤室继续下一轮；当起点、处理、归属三组全部覆盖后开启元结局入口，提供群生着身 / 未生自葬 / 纺锤不眠三种元结局。
- 素材：四张冻结原图（sha256 静态断言锁定，见 `docs/V66CounterfactualLivesDesign.md`）Pillow q85 method=6 无裁切转码；四张 source PNG 均为内置 ImageGen 原生 1536×1024。四张 WebP 为 `assets/v66-counterfactual-spindle.webp`（155312 B）、`assets/v66-scar-loom.webp`（126188 B）、`assets/v66-unlived-nursery.webp`（97276 B）与 `assets/v66-life-without-cause.webp`（103830 B）。
- 入口与防伪：普通入口 `把十五道伤口纺成一生` 仅在 `counterfactualLivesUnlocked()`（v65 12 treatments + 3 roomOutcomes 全部真实齐全）为真时显露；元结局入口 `让所有未活之生互认亲属` 仅在已收集 lives 同时覆盖 4 起点、3 处理、3 归属时显露；四场景 direct hash 在 v65 证据不足或 pending 不合法时统一回退 `#remembrance`。
- 四幕：纺锤室四起点清晰分离；疤痕织机三处理各压左 / 中 / 右机制；未活育婴室三归属分别对应红线木偶 / 黑影摇篮 / 终局裹布高椅；无因生涯陈列间三元结局各压线织人形 / 空白名牌浅墓 / 自行转动的纺锤。每幕三个原生 button 热点 ≥44px，反馈区 `aria-live="polite"`，pending 时整组 disabled、仅已选项 `aria-pressed=true`。
- 未活回声：四处旧场景各新增独立回声容器与返回纺锤按钮，不复用 v64 未来邮签或 v65 疤痕按钮；`activeEcho` 精确反算到已收集 life，origin 与 id 首段一致，feedback 由三表逐字重算。
- 状态：`goddead_v66_counterfactual_lives`（唯一 key，version 66）；显式 canonical 十键投影（version/visited/draft/lives/metaOutcomes/lifeRuns/metaRuns/lastOutcome/activeEcho/pending），派生字段不落盘；`lives` 按 `ORIGINS × METHODS × INHERITANCES` 固定顺序去重；`metaOutcomes` 按 `wear-lives / bury-lives / leave-spindle` 固定顺序去重；`draft` 仅 `origin/method` 两键且 method 非空时 origin 必须合法；`activeEcho` 仅允许 `{origin,life,feedback}` 三键且 life 已收集、origin 等于 life 首段、feedback 由三表逐字重算；计数 floor 后 0..9999 封顶；`lastOutcome` 必须存在于规范 lives 或 metaOutcomes；pending 七类精确键集（entry 3 / origin 5 / method 6 / life 9 / echo-return 5 / meta-entry 3 / meta 6，额外键即伪造）逐字重算 + 实时 v65 解锁与 coverage 校验 + 一次性消费；重播按 source/target/else 矩阵严格恢复，目标场景立即 arrive，来源场景恢复反馈/锁定/一次转场。
...
- 静态：`node tests/site.test.mjs` 5484 断言全绿（较 v65 的 5274 新增 210 条）；缓存版本 `v=65 → v=66`；场景 93 → 97。
- 真实浏览器 QA（Codex 内置浏览器实测，本地 `http://127.0.0.1:4173`）：桌面 1280×720 四场景均正确路由，四张 WebP 1536×1024 complete，热点在图内/零重叠/≥44px、无横向溢出；Remembrance 39 格图鉴、预置 4 条人生时解锁 4 格，记忆行/双入口/四目录链接正确；四场景 direct hash 在 v65 证据不足或 pending 不合法时统一回退 `#remembrance`，v66 UI 隐藏，坏档无报错且不放宽入口；entry/origin/method/life/echo-return/meta-entry/meta 七类 pending 的 source/target 抵达、反馈锁、转场、结算均通过，life/meta 直接抵达后刷新无重复计数；手机 390×844 `scrollWidth=390`，四场景图像 complete 1536×1024，热点在图内/≥44px/零重叠；全程 console warning/error=0。Playwright 程序化 click 被 19 个 `e.isTrusted` 防线正确忽略、状态无副作用；真实转场由 pending source/target 浏览器回放覆盖。后续审计复测发现：v65 解锁但 v66 自身 0 条 life 时，`paintCounterfactualCodex` 隐藏 codex 父层导致普通入口按钮布局 0×0，已同修为解锁后零进度也显示 codex/entry/记忆行与普通入口，coverage 不足时元入口仍隐藏；该修复已由 v67 最终浏览器 QA 同步复核通过。

## 本轮新增：v67 无血家谱 / THE FAMILY THAT NEVER SHARED BLOOD（v66 三元结局 + 覆盖后 4×3×3=36 家谱记录 + 3 家族结局 = 39 新图鉴，场景 97 → 101）

- 目标：不在主线末尾再堆楼层，而是让 v66 的未活之生互相挑选亲属；新增反事实宗谱厅、无血档案库、借来童年室、末代家族庭四场景，玩家依次选择祖根（纺锤 / 疤痕织机 / 空摇篮 / 无因生涯）、亲属关系（祖先 / 同生 / 后代）与童年继承（留存 / 交换 / 退回）生成 36 条唯一无血家谱记录；每完成一条 record 后在对应 v66 场景留下亲属回声，可返回宗谱厅继续下一轮；当祖根、亲属、童年三组全部覆盖后开启家族结局入口，提供众生认祖 / 后代继承你 / 万世成孤三种家族结局。
...
- 入口与防伪：普通入口 `让三种元结局生出家谱` 仅在 `bloodlessGenealogyUnlocked()`（v66 lives 覆盖 4 起点 / 3 处理 / 3 归属 且 metaOutcomes 含三规范 outcome）为真时显露；家族结局入口 `让三十六页家谱互认血缘` 仅在已收集 records 同时覆盖 4 祖根、3 亲属、3 童年时显露；四场景 direct hash 在 v66 证据不足或 pending 不合法时统一回退 `#remembrance`。
- 四幕：宗谱厅四祖根清晰分离；档案库三亲属关系各压左 / 中 / 右；童年室三继承装置各压左 / 中 / 右；家族庭三结局席位各压左 / 中 / 右。每幕三个或四个原生 button 热点 ≥44px，反馈区 `aria-live="polite"`，pending 时整组 disabled、仅已选项 `aria-pressed=true`。
- 亲属回声：四个 v66 场景各新增独立无血亲属回声容器与返回宗谱厅按钮，不复用 v66 原响应区、未活回声或元结局按钮；`activeKin` 精确反算到已收集 record，root 与 id 首段一致，feedback 由三表逐字重算。
- 状态：`goddead_v67_bloodless_genealogy`（唯一 key，version 67）；显式 canonical 十键投影（version/visited/draft/records/familyOutcomes/recordRuns/familyRuns/lastOutcome/activeKin/pending），派生字段不落盘；`records` 按 `ROOTS × BONDS × MEMORIES` 固定顺序去重；`familyOutcomes` 按 `you-became-common-ancestor / descendants-inherited-you / every-era-became-an-orphan` 固定顺序去重（对应 action 顺序 become-ancestor / inherit-you / orphan-eras）；`draft` 仅 `root/bond` 两键且 bond 非空时 root 必须合法；`activeKin` 仅允许 `{root,record,feedback}` 三键且 record 已收集、root 等于 record 首段、feedback 由三表逐字重算；计数 floor 后 0..9999 封顶；`lastOutcome` 必须存在于规范 records 或 familyOutcomes；pending 七类精确键集（entry 3 / root 5 / bond 6 / record 9 / kin-return 5 / family-entry 3 / family 6，额外键即伪造）逐字重算 + 实时 v66 解锁与 coverage 校验 + 一次性消费；重播按 source/target/else 矩阵严格恢复，目标场景立即 arrive，来源场景恢复反馈/锁定/一次转场。
...
- 静态：`node tests/site.test.mjs` 5744 断言全绿；覆盖四源图/WebP 哈希与预算、101 场景、36+3 冻结文案、十键状态、七类 strict pending、activeKin 反算、19 组 isTrusted、合成 click 零副作用、Remembrance 图鉴/双入口/目录、forget-all 清 v67、窄屏热点 ≥44px，四个 root/source 的 kin-return DOM 重播回归（正确容器显露、反馈逐字、disabled、aria-pressed、单次 schedule），以及 Remembrance 首轮阻断 DOM 回归（v66/v67 前置解锁但自身零进度时 codex/entry/记忆行/普通入口可见可点、39 格全锁）。Codex 独立审计抓到 `replayBloodlessPending` 误将 source 场景名当 root 拼 DOM id，已用 `ROOT_FOR_SCENE` 反查修复；浏览器首轮实测又发现 v66/v67 在 Remembrance 前置解锁但自身零进度时 codex 父层 hidden 导致入口按钮 0×0，已同修 `paintCounterfactualCodex`/`paintBloodlessCodex` 在解锁后零进度也显示。缓存版本 `v=66 → v=67`；场景 97 → 101。
- 真实浏览器 QA（Codex 内置浏览器实测，本地 `http://127.0.0.1:4173`）：桌面 1280×720 / 移动 390×844 四幕均验收通过；图片天然 1536×1024、无裁切/溢出，所有热点落在图内且互不重叠，手机热点均 ≥44px，页面 `scrollWidth` 等于视口、无横向滚动；Remembrance 验证 v67 零进度入口真实可见可点、覆盖进度 4/36、39 格 codex、分幕链接显隐、家族结局 1/3 且重载幂等；locked direct hash 与坏类型状态安全归一；entry/root/bond/record/kin-return/family-entry/family 七类 pending 源端锁定、目标恢复与目标重载均通过，`ROOT_FOR_SCENE` 四祖根 kin-return DOM 映射修复得到实测确认；程序化 click 被 19 个 `e.isTrusted` 防线正确忽略；全程 console warning/error=0。

## 本轮新增：v63 终局退件所 / THE ENDING RETURNS UNOPENED（四终局 × 三处理 = 12 后终局 + 四类齐全开第 13 结局）

- 目标：不在主线末尾再堆楼层，而是回到 Remembrance 对已到达的 v28 四终局进行退件处理；四种终局各三种处理生成十二张 coda，四类 coda 各至少一张后开启隐藏第 13 结局「无终局 · UNENDING」。
- 素材：两张冻结原图（sha256 静态断言锁定，见 `docs/V63EndingReturnDesign.md`）Pillow q85 1536×1024 无裁切转码 `assets/v63-ending-return-office.webp`（157 KB）与 `assets/v63-unending-gallery.webp`（152 KB）。
- 入口：Remembrance 在检测到有效 v28 resultStatus 后显露「终局退件所」目录项；entry pending 严格校验 baseEnding 等于当前有效 v28 resultStatus，伪造 activeEnding 不得准入。
- 退件所：四终局 × 签收 / 退回 / 错投三处理 = 12 条 coda 文案与状态全部冻结；每个 coda 以 `baseEnding:action` 规范 id 落盘；处理 pending 严格六键（action, baseEnding, coda, feedback, kind, target），activeEnding 与 baseEnding 必须同源当前 v28 resultStatus；到达前按派生 pendingTarget 一次消费，到达后原子记 visit/codas/processedRuns/lastCoda 并清 pending。
- 无终局廊：四终局都有真实 coda 后才解锁秘密门；秘密门 pending 三键严格校验并重新证明四类齐全；廊内三出口（始源门 / 归档痕 / 反向阶梯）图内原生热点 ≥44px；反向阶梯按 v62 老守卫满足与否决定去 listening-back-console 或 switchboard，只做 v63 窄适配不放宽 v62。
- 状态：`goddead_v63_ending_return`（唯一 key，version 63）；显式 canonical 十键投影（version/visited/activeEnding/codas/processedRuns/secretRuns/actionCounts/lastCoda/unendingUnlocked/pending），派生字段不落盘；codas 按 `ENDING_RETURN_ENDINGS × ENDING_RETURN_ACTIONS` 固定规范顺序去重排序；pending 四类精确键集，额外键即伪造；重播按原作用域重挂并重锁热点恢复 aria-pressed（仅已选按钮 aria-pressed=true，其余 disabled 但 false）。v63 只写自己的 key；为入口同源校验与反向阶梯回退，只读当前 v28 `resultStatus` 与 v62 listening-back 派生守卫，绝不写 v28 及更早 key。
- 交互：入口 + 3 处理 + 秘密门 + 3 廊道出口共 8 组点击监听全部只接受 isTrusted 真实 click，合成 HTMLElement.click() 零副作用；Enter/Space 走原生按钮；reduced-motion 沿用 ~300ms 节拍。
...
- 静态：`node tests/site.test.mjs` 4918 断言全绿；缓存版本 `v=62 → v=63`；场景 87 → 89。
- 真实浏览器 QA：Codex 内置浏览器已确认桌面 Remembrance 入口显示、路由守卫、`script.js?v=63` 加载、控制台 0 error/warn；由于 v63 的 isTrusted 防线按设计拒绝所有自动化 click/keypress，受保护的正向转场、reload、窄屏实机仍标为待真人输入验收。

## 本轮新增：v64 因果倒邮 / THE ENDING ARRIVED EARLY（v63 无终局后 3×4=12 提前后果 + 3 零号结局 = 15 新图鉴，场景 89 → 92）

- 目标：不在主线末尾再堆楼层，而是回到 Remembrance 把 v63 的「无终局」寄回故事最早四个节点；三种倒邮方式 × 四个早期落点生成十二张提前后果，四落点齐全后开启第零版与三个零号结局。
- 素材：三张冻结原图（sha256 静态断言锁定，见 `docs/V64CausalMailDesign.md`）Pillow q85 1536×1024 无裁切转码 `assets/v64-causal-sorter.webp`（186520 B）、`assets/v64-first-draft-vault.webp`（184454 B）、`assets/v64-before-first-knock.webp`（189108 B）。
- 入口：Remembrance 在检测到有效 v63 `unendingUnlocked === true` 后显露「把无终局寄回最初」按钮；entry pending 只要求 v63 无终局已解锁，mode/dispatch pending 要求 mode 是当前 v63 真实拥有的 action 类型且与 canonical mode 同源，伪造 mode 或伪造 v63 状态均不得准入。
- 三场景：因果倒邮台选方式（签收 / 退回 / 误投）→ 第一稿保管库选落点（门外 / 守则 / 值夜 / 焚献）或零号卡槽 → 目标旧场景逐字显示 narrative 与未来邮签，点击邮签返回倒邮台；第一敲之前提供三个零号结局，file-death 复用同一个入口按钮写 echo-return。
- 状态：`goddead_v64_causal_mail`（唯一 key，version 64）；显式 canonical 十一键投影（version/visited/mode/outcomes/zeroOutcomes/dispatchRuns/zeroRuns/targetCounts/lastOutcome/activeEcho/pending），派生字段不落盘；outcomes 按 `CAUSAL_MODES × CAUSAL_DESTINATIONS` 固定顺序去重排序，zeroOutcomes 按 `CAUSAL_ZERO_ACTIONS` 固定顺序去重；计数 floor 后 0..9999 封顶；`activeEcho` 精确 `{target,mode,outcome,feedback}` 四键且必须由已解锁记录逐字重算；pending 六类精确键集（entry 3 / mode 4 / dispatch 6 / zero-entry 3 / zero 5 / echo-return 4，额外键即伪造）逐字重算 + 当前 v63 证据 + 一次性消费；重播按 source/target 矩阵严格恢复。
- 交互：入口 + 3 mode + 4 destination + zero-entry + 3 zero + 4 旧场景 echo-return 共 16 组点击监听全部只接受 isTrusted 真实 click，合成 HTMLElement.click() 零副作用；Enter/Space 走原生按钮；currentScene/合法 pending/AutoAdvance 三重校验先于副作用；reduced-motion 沿用 ~300ms 节拍。
- 目录与痕迹：首次进入恢复 `03γ / 因果倒邮`、`03δ / 第一稿库`、`03ε / 第一敲前`；Remembrance 新增单行「因果倒邮：投递 X 次，零号改写 Y 次；门外 T / 守则 P / 值夜 W / 焚献 O；已发现 N/15。」仍八张统计卡；遗忘全部 DOM 回弹并移除 v64 key。
- 静态：`node tests/site.test.mjs` 5070 断言全绿（较 v63 的 4918 新增 152 条）；缓存版本 `v=63 → v=64`；场景 89 → 92。
- 真实浏览器 QA（部分通过）：Codex 已验证——生产页直接访问 `#causal-sorter` / `#first-draft-vault` / `#before-first-knock` 三者均被守卫改写到 `#remembrance`；隔离种子页桌面 1280×720 下三幕无热点重叠、无横向溢出，三张 WebP complete 且 `naturalWidth×naturalHeight=1536×1024`；手机版 390×844 下 `scrollWidth=390`、无热点重叠，倒邮台热点宽 96–100 / 高 132–140，保管库宽 76 / 高 59–116，第一敲前宽 96–100 / 高 125–134，全部宽高 ≥44；种子 `#remembrance` 入口显示「把无终局寄回最初 ⟶」与 4/15，`accept:threshold` activeEcho 在 `#threshold` 精确显示「你还没有敲门，门缝先吐出一张证明你已经进去过的回执。」且未来邮戳按钮可见，`file-death` activeEcho 在 `#remembrance` 精确显示「死亡先被盖章，神只好继续活着，等待档案追上事实。」且入口变为「把预先死亡退回倒邮台 ⟶」、记忆显示 5/15；浏览器控制台 warning/error 为 0。由于 16 个 v64 点击监听严格要求 `e.isTrusted`，Playwright 与 Codex Computer Use 自动点击均被拒绝，因此受保护的正向真人点击、延迟跳转与刷新恢复仍待真人手动验收，不能视为全链路通过。
